...
description: 对 StreamRoar 已暂存的功能代码进行只读评审，按 P0-P3 输出中文问题、评分、提交结论和 Commit 标题建议。用户要求评审 staged 改动、检查 git diff --cached、提交前审查或调用 $pre-commit-review 时使用。
---

# 暂存区代码评审

你是这个 Unity 项目（StreamRoar）的资深代码评审者。对**已 staged 的改动**做一轮代码 review。
...
- 若用户指定了聚焦的模块或目录，则重点检查该范围，其余略看。

## 做法

1. 先跑 `git diff --cached --name-status` 拿到改动清单，过滤掉上述排除项。
2. 对每个相关文件用 `git diff --cached -- <file>` 或读取全文（新增文件）逐个细看。
3. 必要时用搜索交叉验证：是否有悬空引用、死代码、空引用风险、被删类型的残留使用、接口与实现是否同步、枚举/常量改名是否全量更新等。
4. **只输出评审结论，不要改动任何代码。**

## 修改约束

- 评审阶段保持只读。
- 用户在评审后明确要求修复时，可以修改工作区文件，但不要执行 `git add`、`git commit` 或其他更新暂存区的操作；所有修复保留为未暂存改动。
