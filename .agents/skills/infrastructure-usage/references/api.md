...
按表契约获取运行时配置表。当前 ApplicationController 尚未构造 Tables 或注册 IConfigProvider；接入前需由组合根完成装配。
...
| `Release(instance)` | 归还本池已租借实例；重复或外部有效实例会报错，null/已销毁实例为空操作。 |
...
控制光标锁定与显示。当前 ApplicationController 尚未注册或驱动 ICursorService；使用时需由组合根接入启动、帧更新和焦点变化。
